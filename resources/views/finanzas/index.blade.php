...
<div class="space-y-8 animate-fade-in" x-data="{ activeTab: 'membresias' }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-2">
            @if(auth()->user()->hasPermission('finanzas.export_reports'))
            ...
            @endif
            @if(auth()->user()->hasPermission('finanzas.record_payment'))
            ...
            @endif
            @if(auth()->user()->hasPermission('finanzas.plans_manage'))
            ...
            @endif
            @if(auth()->user()->hasPermission('finanzas.record_payment') || auth()->user()->hasPermission('finanzas.plans_manage'))
        ...
        <form id="abono-form" action="{{ route('finanzas.record_abono') }}" method="POST" onsubmit="submitAbonoForm(event)" class="space-y-4">
            ...
            <div id="abono_preview_box" class="bg-slate-950 p-4 rounded-2xl border border-slate-850 space-y-2.5 text-xs">
                ...
                <div class="flex items-center justify-between text-slate-400 border-b border-slate-850/60 pb-2">
                    ...
                    <span id="abono_preview_prev_credit" class="font-bold text-amber-400">$0.00</span>
                </div>
                <div class="flex items-center justify-between text-slate-300 border-b border-slate-850/60 pb-2 font-bold">
                    ...
                    <span id="abono_preview_total_funds" class="font-black text-slate-100">$0.00</span>
                </div>
                <div class="flex items-center justify-between text-slate-400 border-b border-slate-850/60 pb-2">
                    <span class="font-semibold">Estado de Renovación:</span>
                    <span id="abono_preview_status_pill" class="px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider bg-amber-500/10 text-amber-400 border border-amber-500/20">Acumulando Saldo</span>
                </div>
                <div class="flex items-center justify-between text-slate-400 border-b border-slate-850/60 pb-2" id="abono_preview_missing_row">
                    <span class="font-semibold">Falta para 1 Mes Completo:</span>
                    <span id="abono_preview_missing_amount" class="font-bold text-rose-400">$0.00</span>
                </div>
                <div class="flex items-center justify-between text-slate-400 border-b border-slate-850/60 pb-2">
                    <span class="font-semibold">Saldo a Favor Restante:</span>
                    <span id="abono_preview_new_credit" class="font-bold text-amber-400">$0.00</span>
                </div>
...
<script>
    ...
    function calculateAbonoPreview() {
        ...
        const totalFundsEl = document.getElementById('abono_preview_total_funds');
        const statusPillEl = document.getElementById('abono_preview_status_pill');
        const missingRowEl = document.getElementById('abono_preview_missing_row');
        const missingAmountEl = document.getElementById('abono_preview_missing_amount');
        ...
        if (!select || !select.value) {
            ...
            if (totalFundsEl) totalFundsEl.textContent = '$0.00';
            if (statusPillEl) {
                statusPillEl.textContent = 'Selecciona Membresía';
                statusPillEl.className = 'px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider bg-slate-800 text-slate-400 border border-slate-700';
            }
